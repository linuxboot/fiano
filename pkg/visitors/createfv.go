...
func (v *CreateFV) Visit(f uefi.Firmware) error {
	...
	L:
		...
		v.found = true
		return v.createFV(f, offset, bp, idx)
	}

	return f.ApplyChildren(v)
}

func (v *CreateFV) createFV(br *uefi.BIOSRegion, offset uint64, bp *uefi.BIOSPadding, idx int) error {
	...
	if offset+bp.Offset < v.AbsOffset {
		hbp, err := uefi.NewBIOSPadding(bpBuf[:v.AbsOffset-offset-bp.Offset], bp.Offset)
		if err != nil {
			return err
		}
	...
	fv.Blocks[0] = uefi.Block{Size: 4096, Count: uint32(v.Size / 4096)}
	...
	if err != nil {
		return fmt.Errorf("unable to construct binary header of new firmware volume: got %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("unable to construct binary extended header of new firmware volume: got %v", err)
	}

	extHeaderFile, err := uefi.CreatePadFile(uint64(uefi.FileHeaderMinLength + fv.ExtHeaderSize))
	if err != nil {
		return fmt.Errorf("Building ExtHeader %v", err)
	}
	...
	if err = fv.InsertFile(fv.DataOffset, extHeaderFileBuf); err != nil {
		return fmt.Errorf("Adding ExtHeader %v", err)
	}
	...
	// Internal fields
	fv.FVOffset = v.AbsOffset - offset
	...
	if v.AbsOffset-offset-bp.Offset+v.Size < uint64(len(bpBuf)) {
		tbp, err := uefi.NewBIOSPadding(bpBuf[v.AbsOffset-offset-bp.Offset+v.Size:], v.AbsOffset-offset+v.Size)
		if err != nil {
			return err
		}
